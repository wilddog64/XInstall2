						    ...
						    if ( CheckForNode != null )
							    if ( this._RegExCheckNotContains.IsMatch( CheckForNode.Value ) )
								    base.FatalErrorMessage( ".",
											    String.Format(
												"{0} : section {1}, key {2} value : {3} contains invaliad value {4}",
												this.XmlFilePath, this.Section,
												SearchForNode.Value, CheckForNode.Value,
												this.CheckNotContains ), 1660, false );
					    }
				    }
			    }
		    }
