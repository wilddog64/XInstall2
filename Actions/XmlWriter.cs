...
namespace XInstall.Core.Actions {
    ...
    public class XmlWriter : ActionElement {
	    ...
	    private WEBCONFIG_OPR_CODE _WebConfOprCode = WEBCONFIG_OPR_CODE.WEBCONFIG_OPR_SUCCESS;
	    ...
	    public string WebConfigFile {
		    ...
		    set {
			    ...
			    {
				    this.SetExitMessage( WEBCONFIG_OPR_CODE.WEBCONFIG_OPR_CONFIG_FILE_NOTFOUND,
	    ...
	    private void SetExitMessage( WEBCONFIG_OPR_CODE WebConfigCode, params object[] objParams ) {
	    ...
	    private Hashtable GetUpdateInfo( XmlNodeList UpdateNodes ) {
		    ...
		    foreach ( XmlNode xn in UpdateNodes ) {
			    ...
			    if ( xn.Name.Equals( @"modifykey" ) ) {
				    ...
				    if ( XPath == null ) {
					    this.SetExitMessage( WEBCONFIG_OPR_CODE.WEBCONFIG_OPR_EMPTY_UPDATE_PARAMETERS, this.Name, MethodName, "key" );
				    }
				    if ( Value == null ) 
