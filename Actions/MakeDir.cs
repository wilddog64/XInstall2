	    ...
	    protected override void ParseActionElement() {
		    ...
		    try {
			    if ( Directory.Exists( this.DirectoryName ) ) {
				    base.LogItWithTimeStamp( string.Format("{0}: Directory {1} is already existed!", this.Name, this.DirectoryName) );
			    }
			    else {
				    Directory.CreateDirectory( this.DirectoryName );
				    base.LogItWithTimeStamp( String.Format( "{0}: Directory {1} is created", this.Name, this.DirectoryName ) );
			    }
		    }
		    catch ( Exception e ) {
