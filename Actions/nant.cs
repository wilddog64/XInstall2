...
namespace XInstall.Core.Actions {
    ...
    public class Nant : ActionElement, IAction {
	    ...
	    public string BuildFile {
		    ...
		    set {
			    ...
			    if ( !File.Exists( this._BuildFile ) ) {
				    base.FatalErrorMessage( ".", String.Format( @"file {0} is not found!", this._BuildFile ), 1660, -1 );
			    }
		    }
	    }
	    ...
	    private void ExecuteNant( params string[] parameters ) {
		    ...
		    MethodInfo   mi       = t.GetMethod( @"Main", BindingFlags.Public | BindingFlags.Static, null, new Type[] { typeof( string[] ) }, null);
