    ...
    public class RegKey : ActionElement
    {
	    ...
	    public string Action
	    {
		    set
		    {
			    ...
			    default:
				    this._iExitCode =
					(int) REGKEY_OPERATION_ERROR.REGKEY_UNKNOWN_ACTION;
				    break;
			    }
		    }
	    }
	    ...
	    protected override void ParseActionElement()
	    {
		    ...
		    // if necessary.
		    if ( this._iExitCode ==
			    (int) REGKEY_OPERATION_ERROR.REGKEY_OPERATION_SUCCESS )
		    {
			    this.OperationName =
				this._strActions[ (int) this._enumAction ];
			    this.GetExitMessage();
		    }
		    else if ( this._iExitCode ==
			      (int) REGKEY_OPERATION_ERROR.REGKEY_MACHINE_NOTFOUND )
		    {
			    this.OperationName =
				this._strActions[ (int) this._enumAction ];
			    this.GetExitMessage();
	    ...
	    private void OpenRegKey()
	    {
		    ...
		    {
			    this._iExitCode =
				( int ) REGKEY_OPERATION_ERROR.REGKEY_USER_NO_ACCESSRIGHT;
		    }
	    }
	    ...
	    private void SetSubkeyValue( RegistryKey rk )
	    {
		    ...
		    {
			    this._iExitCode =
				(int) REGKEY_OPERATION_ERROR.REGKEY_NULLVALUE;
		    }
		    catch ( System.UnauthorizedAccessException )
		    {
			    this._iExitCode =
				(int) REGKEY_OPERATION_ERROR.REGKEY_READONLY_SUBKEY;
		    }
		    catch ( System.ObjectDisposedException )
		    {
			    this._iExitCode =
				(int) REGKEY_OPERATION_ERROR.REGKEY_HKEY_CLOSED;
		    }
		    catch ( System.Security.SecurityException )
		    {
			    this._iExitCode =
	    ...
	    private RegistryKey CreateSubKey( RegistryKey rk )
	    {
		    ...
		    {
			    this._iExitCode =
				(int) REGKEY_OPERATION_ERROR.REGKEY_NOSUBKEY_PROVIDED;
		    }
		    catch ( System.Security.SecurityException )
		    {
			    this._iExitCode =
				(int) REGKEY_OPERATION_ERROR.REGKEY_CANNOT_CREATEKEY;
		    }
		    catch ( System.UnauthorizedAccessException )
		    {
			    this._iExitCode =
				(int) REGKEY_OPERATION_ERROR.REGKEY_UNAUTH_CREATEKEY;
		    }
		    catch ( System.ObjectDisposedException )
		    {
			    this._iExitCode =
	    ...
	    private RegistryKey OpenSubKey ()
	    {
		    ...
		    return this._enumAction == REGKEY_ACTION.REGKEY_CREATE ?
			   this.CreateSubKey( rkThisKey )                     :
			   rkThisKey;
	    ...
	    private void DeleteSubKey()
	    {
		    ...
		    {
			    this._iExitCode =
				(int) REGKEY_OPERATION_ERROR.REGKEY_KEY_HAS_SUBTREE;
			    rk.DeleteSubKeyTree( this.Path );
		    }
		    catch ( System.ArgumentNullException )
		    {
